...
export async function createDebtsLoansBucket() {
  try {
    ...
    if (buckets?.some((bucket) => bucket.name === "debts_loans_documents")) {
      return { success: true, message: "Bucket already exists" }
    }

    // Create the bucket
    const { data, error } = await adminClient.storage.createBucket("debts_loans_documents", {
      public: false,
    })
...
async function setupBucketPolicies(adminClient: any) {
  try {
    ...
    await adminClient.query(`
      ...
      ON storage.objects
      FOR SELECT
      USING (bucket_id = 'debts_loans_documents' AND auth.uid()::text = (storage.foldername(name))[1]);
    `)
    ...
    await adminClient.query(`
      ...
      ON storage.objects
      FOR INSERT
      WITH CHECK (bucket_id = 'debts_loans_documents' AND auth.uid()::text = (storage.foldername(name))[1]);
    `)
