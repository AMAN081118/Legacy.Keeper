...
async function setupBucketPolicies(adminClient: any) {
  try {
    ...
    await adminClient.query(`
      CREATE POLICY IF NOT EXISTS "Users can view their own debts-loans-documents files"
      ON storage.objects
      FOR SELECT
    ...
    await adminClient.query(`
      CREATE POLICY IF NOT EXISTS "Users can upload their own debts-loans-documents files"
      ON storage.objects
      FOR INSERT
    ...
    await adminClient.query(`
      CREATE POLICY IF NOT EXISTS "Users can update their own debts-loans-documents files"
      ON storage.objects
      FOR UPDATE
      USING (bucket_id = 'debts-loans-documents' AND auth.uid()::text = (storage.foldername(name))[1]);
    `)
    ...
    await adminClient.query(`
      CREATE POLICY IF NOT EXISTS "Users can delete their own debts-loans-documents files"
      ON storage.objects
      FOR DELETE
      USING (bucket_id = 'debts-loans-documents' AND auth.uid()::text = (storage.foldername(name))[1]);
    `)
